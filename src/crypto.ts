...
const key = await webcrypto.subtle.deriveKey({
    ...
    name: 'AES-CBC',
    length: 256,
}, true, ['encrypt', 'decrypt', 'unwrapKey', 'wrapKey']);
...
function ivAndCiphertextToString({iv, ciphertext}: { iv: ArrayBuffer, ciphertext: ArrayBuffer}) {
    return JSON.stringify([ui8ToString(new Uint8Array(ciphertext), 'base64'), ui8ToString(new Uint8Array(iv), 'base64')]);
}
