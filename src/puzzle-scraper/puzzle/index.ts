...
export async function generateSignature(input:
    ...
    function computeAverageSampleLuminosities(
        ...
        const squareCenters = [...computeSquareCenters(sharpBuf)];

        const sampleLuminosities = new Array<number>(squareCenters.length);
        for (let i = 0; i < squareCenters.length; i++) {
            const squareCenter = squareCenters[i];
            sampleLuminosities[i] = computeSquareAverage(image, pixelArray, squareCenter, squareSize);
        }
    ...
    function computeSquareAverage(
        ...
        const [squareX, squareY] = squareCenter;
        const squareCorner = [
    ...
    function* computeSquareCenters({ info: { width, height } }: {
        data: Buffer;
        ...
        for (let x = 0; x < gridSize; ++x) {
            for (let y = 0; y < gridSize; ++y) {
                yield [
                    Math.round(widthOffset * (x + 1)),
                    Math.round(heightOffset * (y + 1))
                ];
            }
        }
    }

    function computeNeighbourDifferences(luminosityAverages: number[]) {
        ...
        const neighbourDifferences = new Array<number>(gridSize * gridSize * 8);
        let spandex = 0;
    ...
    function computeRelativeLuminosityLevels(neighbourDifferences: number[]) {
        const darks: number[] = [];
        const lights: number[] = [];
        ...
        for (const difference of neighbourDifferences) {
            ...
            if (difference < noiseCutoff) {
                darks.push(difference);
                continue;
            }

            if (difference > noiseCutoff) {
                lights.push(difference);
            }
        }

        const muchDarkerCutoff = darks.length > 0 ? median(darks) : -noiseCutoff;
        const muchLighterCutoff = lights.length > 0 ? median(lights) : noiseCutoff;

        const luminosityLevels = new Array<LuminosityLevel>(neighbourDifferences.length);
 ...
 * @param levels
 */
export function encodeToBitArray(levels: LuminosityLevel[]): ArrayBuffer {
...
export enum SignatureSimilarity {
...
function euclideanLength(signature: ArrayLike<LuminosityLevel> & Iterable<LuminosityLevel>) {
    let sum = 0.0;
