...
export default class Gelbooru extends Booru<number, GelbooruPost> {
    ...
    async getPost(id: number, options?: { fullpost?: boolean }): Promise<GelbooruPost | PartialPost> {
        const post = (await this.baseHandler.get(`${this.baseUrl}/index.php?page=dapi&s=post&q=index&id=${id}`, { xml: true })) as GelbooruResult;
    ...
    makePartialPost(post: GelbooruPost): PartialPost {
        ...
        return {
            ...
            thumbnail_image: [post.sample_url, post.preview_url, post.file_url].filter(e => e !== undefined).filter(e => e),
    ...
    async search(after: number = 0, limit: number = 1000): Promise<PartialPost[]> {
        ...
        return await this.baseHandler.get(`${this.baseUrl}/index.php?page=dapi&s=post&q=index&tags=${encodeURIComponent(`sort:id:asc id:>${after}`)}`, { xml: true })
            .then(e => (e as GelbooruResult).posts.post.map(e => this.makePartialPost(e)));
    }

    async getLastPostId(): Promise<number> {
        const posts = (await this.baseHandler.get(`${this.baseUrl}/index.php?page=dapi&s=post&q=index&limit=1`, { xml: true })) as GelbooruResult;
