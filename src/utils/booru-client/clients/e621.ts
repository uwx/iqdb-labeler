...
export default class E621 extends Booru<number, E621Post> {
    ...
    async getPost(id: number, options?: { fullpost?: boolean }): Promise<E621Post | PartialPost> {
        const { post } = (await this.baseHandler.get(`https://e621.net/posts/${id}.json`, {
    ...
    makePartialPost(post: E621Post): PartialPost {
        const partialPost = {
            ...
            image: [post.file?.url, ...(post.sample?.has ? [post.sample?.url] : [])]
        ...
        if (
            ...
            (partialPost.tags.includes('general:loli') ||
                partialPost.tags.includes('general:shota') ||
                partialPost.tags.includes('general:young') ||
                partialPost.tags.includes('general:cub'))
        ) {
    ...
    async search(after: number = 0, limit: number = 320): Promise<PartialPost[]> {
        ...
        return await this.baseHandler
            .get(`https://e621.net/posts.json?limit=${limit}&page=a${after}`, { userAgent: this.userAgent })
            .then((e) => ((e as { posts: E621Post[] }).posts as E621Post[]).map((e) => this.makePartialPost(e)));
    }

    async getLastPostId(): Promise<number> {
        const posts = (await this.baseHandler.get(`https://e621.net/posts.json?limit=1`, {
            userAgent: this.userAgent,
        })) as E621Post[];
        return posts[0].id!;
    }
