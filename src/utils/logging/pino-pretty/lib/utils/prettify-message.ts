...
import interpretConditionals from './interpret-conditionals';
import type { PrettyContext } from './parse-factory-options';

interface PrettifyMessageParams {
    /** The log object with the message to colorize. */
    log: object;
 ...
 */
export default function prettifyMessage({ log, context }: PrettifyMessageParams): undefined | string {
    ...
    if (messageFormat && typeof messageFormat === 'string') {
        ...
        const message = String(parsedMessageFormat).replace(/{([^{}]+)}/g, (match, p1) => {
